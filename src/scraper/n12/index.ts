...
export class N12 extends BaseScraper {
    ...
    async scrapeChat() {
        ...
        for (const report of reports) {
            ...
            console.log(`We're calling callbacks because item time ${reportTime.toMillis()} (${reportTime.hour}:${reportTime.minute}:${reportTime.second}) is bigger than current time ${currentTime.toMillis()} (${currentTime.hour}:${currentTime.minute}:${currentTime.second})`);
